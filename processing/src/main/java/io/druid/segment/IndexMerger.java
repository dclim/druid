  ...
  ) throws IOException
  {
    ...
    try {
      ...
      for (int i = 0; i < mergedDimensions.size(); i++) {
        DimensionMerger merger = handlers[i].makeMerger(
            indexSpec,
            v8OutDir,
            ioPeon,
            null,
      ...
      for (DimensionMerger merger : mergers) {
        merger.closeWritersV8(rowNumConversions, out, spatialOut);
      }
          ...
          new Function<Rowboat, Rowboat>()
          {
            ...
            public Rowboat apply(@Nullable Rowboat input)
            {
              ...
              for (int i = 0; i < convertedDims.size(); ++i) {
                ...
                if (dims[i] == null) {
                  if (mergers.get(i).needsConvertMissingValues()) {
                ...
                for (int j = 0; j < dims[i].length; ++j) {
                  newDims[i][j] = mergers.get(i).getConvertedEncodedValue(dims[i][j], indexNumber);
                }
              }
